...
--    _log ("file length = "..#info, "openLuup.HTTP.FILE")
    f: close ()
    -- @explorer:  2016.04.14
    -- Workaround for SONOS not liking chunked MP3 and some headers.       
    -- https://en.community.sonos.com/advanced-setups-229000/chunked-encoding-transfer-and-mp3--42082
    -- The SONOS plugin uses consistent file name we can match on: Say.*.mp3.   
    -- Considered checking user-agent but decided not to, as UA can change, whereas the file name generated by the plugin is unlikely to change. 
    -- headers["USER-AGENT"] == "Sonos"
     if path:match("Say.%d+.mp3") then
      chunked = false
      response_headers = {}
    end
